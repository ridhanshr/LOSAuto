...
def update_accnum(no_aplikasi_data, server='172.24.141.60,1433', database='LOSBRICC', username='sa', password='P@ssw0rd'):
    """
    Update the A4_ACHACCT column in LOSBRICC database.
    ...
    try:
        ...
        # SQL query
        sql_query = f"""
            UPDATE APPOPTION SET AD_ACCNUM = 0 WHERE AP_REGNO = '{no_aplikasi_data}'
        """
        sql_query2 = f"""
            SELECT * FROM APPFLAG WHERE AP_REGNO = '{no_aplikasi_data}'
        """
...
def update_applicant(no_aplikasi_data, server='172.24.141.60,1433', database='LOSBRICC', username='sa', password='P@ssw0rd'):
    # Setup DB connection string
    conn_str = (
        f'DRIVER={{ODBC Driver 18 for SQL Server}};'
        f'SERVER={server};'
    ...
    try:
        ...
        # SQL query
        sql_query = f"""
            UPDATE CBASSLIK.dbo.applicant SET status = '4' WHERE appid = '{no_aplikasi_data}'
        """
...
def update_paramater_key(no_aplikasi_data, ch_code, custcatid, pr_code, server='172.24.141.60,1433', database='LOSBRICC', username='sa', password='P@ssw0rd'):
    # Setup DB connection string
    conn_str = (
        f'DRIVER={{ODBC Driver 18 for SQL Server}};'
        f'SERVER={server};'
